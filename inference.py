...
parser.add_argument(
    "--sr_path",
    type=str,
    help="Name of saved checkpoint of super-resolution network",
    required=True,
)
...
parser.add_argument(
    "--gt_path",
    type=str,
    help="Where to store saved ground truth frames",
    required=False,
)
parser.add_argument(
    "--pred_path",
    type=str,
    help="Where to store frames produced by algorithm",
    required=False,
)
parser.add_argument(
    "--save_as_video",
    action="store_true",
    default=False,
    help="Whether to save frames as video",
    required=False,
)
parser.add_argument(
    "--image_prefix",
    type=str,
    default="",
    help="Prefix to save frames with",
    required=False,
)
...
def main():
    ...
    if args.save_as_video:
        gt_out = cv2.VideoWriter(
            "temp/gt.avi", cv2.VideoWriter_fourcc(*"DIVX"), fps, (384, 384)
        )
        pred_out = cv2.VideoWriter(
            "temp/pred.avi", cv2.VideoWriter_fourcc(*"DIVX"), fps, (96, 96)
        )
    ...
    command = "ffmpeg -hide_banner -loglevel error -y -i {} -i {} -strict -2 -q:v 1 {}".format(
    ...
    if args.save_frames and args.save_as_video:
        ...
        command = "ffmpeg -hide_banner -loglevel error -y -i {} -i {} -strict -2 -q:v 1 {}".format(
            args.audio, "temp/gt.avi", args.gt_path
        )
        subprocess.call(command, shell=platform.system() != "Windows")

        command = "ffmpeg -hide_banner -loglevel error -y -i {} -i {} -strict -2 -q:v 1 {}".format(
            args.audio, "temp/pred.avi", args.pred_path
        )
        subprocess.call(command, shell=platform.system() != "Windows")
